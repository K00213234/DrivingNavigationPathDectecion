...
public class MatVectorDataImageGeneratorV6 implements VectorDataImageGenerator
{
	...
	private List<Point> pointArray1 = new ArrayList<Point>();// These should be an array of lines
	...
	{
		imageDataMat = dectectLinesInImageData(imageDataMat);
			...
			List<Point> pointArray)// addingPolarCoridanteSytem
	{
		...
		{
			double rho = houghLinesMat.get(index, 0)[0];// rho is
			double theta = houghLinesMat.get(index, 0)[1];
			double a = Math.cos(theta);
			double b = Math.sin(theta);
			double x0 = a * rho;
			double y0 = b * rho;
			final int lineLenght = 10000;
			Point pointAhead = new Point(Math.round(x0 + lineLenght * (-b)), Math.round(y0 + lineLenght * (a)));
			Point pointBehind = new Point(Math.round(x0 - lineLenght * (-b)), Math.round(y0 - lineLenght * (a)));
...
//							else intersect = new Point(0, 0);
		...
		{
			Point pointOnLine = (getPointAtAngle(intersectionPoint, avgangle)).clone();
			Point poline180 = new Point();
			Point polinem90 = new Point();
			...
			if ((avgangle < 60) || avgangle > 300)
			{
				...
				String direction = getCarDirection(avgangle);
				directCar(direction);

				poline90 = getPointAtAngle(intersectionPoint, (avgangle + 90)).clone();
				if (avgangle > 180)
				{
					// log("point = avgangle - 180 intersect= " + intersectionPoint + " avgangle=" +
					// (avgangle - 180));
					poline180 = getPointAtAngle(intersectionPoint, (avgangle - 180)).clone();
				}
				else
				{
					System.out.print(
							"point = avgangle + 180 intersect= " + intersectionPoint + " avgangle=" + (avgangle + 180)
					);
					poline180 = getPointAtAngle(intersectionPoint, (avgangle + 180)).clone();

				}
			}
			Point poline270 = getPointAtAngle(intersectionPoint, (avgangle + 270));
			...
			polinem90 = getPointAtAngle(intersectionPoint, (avgangle + 180)).clone();
			System.out.print(
					"intersection point = " + intersectionPoint + " avgangle=" + (avgangle + 180) + "pointonline"
							+ pointOnLine
			);
			poline90 = getPointAtAngle(intersectionPoint, (avgangle + 90)).clone();
			Imgproc.line(
					imageDataMat, intersectionPoint, pointOnLine, new Scalar(255, 255, 255), 1, Imgproc.LINE_AA, 0
			);
...
//Slope 45
		...
		// log("pos future " + pointIntheDistance,
		// CameraUserDisplay.LogService.DebuggingAtAngleToken);
